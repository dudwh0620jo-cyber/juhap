...
const sakeDaiginjoItems: CategoryListItem[] = sakeProductsMock.map((product) => ({
  ...
  name: product.name,
  imageSrc:
    product.id === "sake-dassai-23"
      ? imgDassai23
      : product.id === "sake-kubota-manju"
        ? imgKubotaManjyu
        : product.id === "sake-kamoshibito-kuheiji"
          ? imgKuheiji
          : product.id === "sake-hakkaisan-daiginjo"
            ? imgHakkaisanDaiginjo
            : product.id === "sake-nabeshima-daiginjo"
              ? imgNabeshimaDaiginjo
              : product.id === "sake-denshu-junmai-daiginjo"
                ? imgDenshuJunmaiDaiginjo
                : product.id === "sake-juyondai-series"
                  ? imgJuyondaiSeries
                  : product.id === "sake-gekkeikan-horin"
                    ? imgGekkeikanHorin
                    : product.id === "sake-onnanakase"
                      ? imgOnnanakase
                      : undefined,
